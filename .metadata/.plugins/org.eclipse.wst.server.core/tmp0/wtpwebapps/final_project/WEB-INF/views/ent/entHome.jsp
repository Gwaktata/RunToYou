				...
				<tbody id="tbdy">
					<c:forEach var="waitPnt" items="${waitPnt}" varStatus="status">
						<tr role="row" id="${status.count}">
							...
							<td>${waitPnt.pntHp}</td>
							<c:if test="${waitPnt.pntSex eq 'M'}"><td><span class="badge-pill badge-primary">남</span></td></c:if>
							<c:if test="${waitPnt.pntSex eq 'F'}"><td><span class="badge-pill badge-danger">여</span></td></c:if>
							<td>${waitPnt.chrDr}</td>
