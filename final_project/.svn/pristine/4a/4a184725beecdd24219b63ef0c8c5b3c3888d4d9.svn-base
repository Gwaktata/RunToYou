...
// 					fnSelectListSoogiSchSuccess(res);
				}
			})
		}
...
// 			location.href = "/medical/cam?" + strParam;
			...
			// 팝업으로 대체 
			window.open("/medical/cam?" + strParam, "촬영", "width=1280, height=720, top=100, left=100");
			
			// 환자사진 목록 조회(팝업창 닫을 때 호출해야 함)
// 			fnSelectListAllCam();
		...
		$('#vitalCmplBtn').click(function () {
			...
			if (btnTxt == "다시측정") {
				...
				if ($('#height').val() == "") {
					swal("", "신장을 입력해주세요.", "warning");
					$('#height').focus();
					return false;
				}

				if ($('#weight').val() == "") {
					swal("", "체중을 입력해주세요.", "warning");
					$('#weight').focus();
					return false;
				}
...
// 			var srcChk = $('#patient-img').attr('src');
// 			if (srcChk.indexOf('base64') == -1) {
// 				swal("", "환자이미지를 선택해 주세요.", "warning");
// 				return false;
// 			}

// 			if ($('#disCd').text() == "") {
// 				swal("", "병명을 검색 후 선택해주세요.", "warning");
// 				$('#jindanSchTxt').focus();
// 				return false;
// 			}
...
// 				if ($('#drugNm').text() == "") {
// 					swal("", "처방을 검색 후 선택해 주세요.", "warning");
// 					$('#jindanSchTxt').focus();
// 					return false;
// 				}
				for(var i = 0; i < $("#drugTable").find('tr').length; i++){
					if ( $($(".drugTimes")[i]).val() == "") {
						swal("", "일일 복용횟수를 입력해 주세요.", "warning");
						$($(".drugTimes")[i]).focus();
						return false;
					}
...
// 				, 'sgCdArr': sgCdArr
				, 'stateCd': $('#setStateCd').val() != '' ? $('#setStateCd').val() : 'PS6' // 추가검사 버튼을 클릭 안 하면 기본 값은 '수납대기'
			}
	...
	$(document).on("click", "#diagnose3 > ul > li", function () {
		...
		$("#jindanSchResult").addClass("d-none");
// 		$('#drugNm').text($(this).text());
// 		$('#drugCd').val($(this).find('a').data("code"));
// 		$("#jindanSchResult").addClass("d-none");
		// $('#jbqo').text($(this).text());
	});
	...
	window.fnSelectListAllCam = () => {
		fnSelectListCam('환자사진');
// 		fnSelectListCam('검사사진');
	}

	// 환자사진 목록조회 함수
	const fnSelectListCam = (imgType) => {
		fnCommonAjax('post', { 'rcptNo': $('#setRcptNo').val()}, '/medical/selectListCam', fnSelectListCamSuccess1);
// 		fnCommonAjax('post', { 'rcptNo': $('#setRcptNo').val(), 'fileCls': '검사사진' }, '/medical/selectListCam', fnSelectListCamSuccess2);
	}
		...
		if (dummys.length > 0) {
			...
			for (let dummys of splitedDummys) {
				...
				for (let dummy of dummys) {
					...
					if (dummy.fileCls != '환자사진') {
						clsFlag = false;
					}
				}
				
				if (clsFlag) {
					result1 += outerDiv.outerHTML; // innerHTML은 자신을 기준으로 부모 안 나옴, outerHTML은 자신을 기준으로 부모 포함 다 나옴
				} else {
					result2 += outerDiv.outerHTML; // innerHTML은 자신을 기준으로 부모 안 나옴, outerHTML은 자신을 기준으로 부모 포함 다 나옴
				}
			}

		} else {
			result1 += '<div>사진이 없습니다.</div>'
			result2 += '<div>사진이 없습니다.</div>'
		}
		$('#img-list').html(result1);
		$('#img-list2').html(result2);
